...
切换 Provider 只需修改 `PROVIDER` 环境变量（旧版 `SPARK_MODE` 仍兼容）。推荐在应用配置中使用对应 Provider 的专用凭证变量，通用 `API_KEY` 作为兜底：

```env
# 讯飞 Coding Plan
PROVIDER=xfyun-coding
SPARK_API_KEY=your-xfyun-key
...
> - 讯飞 Coding Plan（`xfyun-coding`）：`SPARK_API_KEY` → `API_KEY`
> - 火山方舟（`volcengine-coding`）：`VOLCENGINE_API_KEY` → `API_KEY`
> - 千帆 Coding Plan（`qianfan-coding`）：`QIANFAN_API_KEY` → `API_KEY`
> - 商汤 SenseNova（`sensenova`）：`SENSENOVA_API_KEY` → `API_KEY`
> - DeepSeek（`deepseek`）：`DEEPSEEK_API_KEY` → `API_KEY`
> - OpenCode Go（`opencode-go`）：`OPENCODE_API_KEY` → `API_KEY`
>
> 应用配置优先使用当前 Provider 的专用变量；未设置时才回退到 `API_KEY`。
...
export SPARK_API_KEY=your-xfyun-key
claude
```
...
| `OPENCODE_API_KEY` | - | OpenCode Go 专用 API Key；优先于 `API_KEY` |
...
| `HTTP_PROXY_USER` | ❌ | 代理认证用户名；可不提供密码 |
| `HTTP_PROXY_PASSWORD` | ❌ | 代理认证密码；提供时必须同时提供 USER |
| `HTTPS_PROXY_HOST` | ✅ | HTTPS scheme 代理主机 |
| `HTTPS_PROXY_PORT` | ✅ | HTTPS scheme 代理端口 |
| `HTTPS_PROXY_USER` | ❌ | 代理认证用户名；可不提供密码 |
| `HTTPS_PROXY_PASSWORD` | ❌ | 代理认证密码；提供时必须同时提供 USER |
