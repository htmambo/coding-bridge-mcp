...
even when the console shows quota remaining. The profile default is now
4096 (sits at the wall), so this test keeps pinning ``MCP_MAX_TOKENS=4096``
...
def _build_sensenova_settings(monkeypatch):
    ...
    # 429 "Workspace allocated quota exceeded"). The profile default is
    # already 4096; pin explicitly here as a regression guard.
